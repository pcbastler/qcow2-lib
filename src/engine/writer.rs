...
use crate::format::constants::L2_ENTRY_SIZE;
...
impl<'a> Qcow2Writer<'a> {
    ...
    fn write_cluster_chunk(&mut self, buf: &[u8], guest_offset: u64) -> Result<()> {
        let (l1_index, l2_index, intra) = GuestOffset(guest_offset).split(self.cluster_bits);
        let cluster_size = 1u64 << self.cluster_bits;
    ...
    pub fn write_compressed_at(
        ...
        let (l1_index, l2_index, _intra) =
            GuestOffset(guest_offset).split(self.cluster_bits);
...
mod tests {
    ...
    fn cow_l2_table_preserves_existing_entries() {
        ...
        let new_l2 = L2Table::read_from(&l2_buf, CLUSTER_BITS).unwrap();
