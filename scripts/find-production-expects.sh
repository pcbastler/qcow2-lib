#!/usr/bin/env bash
# Find .expect() calls in production code (excluding tests, examples, benches).
#
# Usage: ./scripts/find-production-expects.sh [crates/]
...
while IFS= read -r -d '' file; do
    ...
    # Print lines with .expect() that are not in comments or doc strings
    /\.expect\(/ && !/^[[:space:]]*\/\// { print FILENAME ":" NR ": " $0 }
    ' "$file"
done
